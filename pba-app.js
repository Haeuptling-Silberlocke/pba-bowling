...
function renderPage(){
  ...
  if(mobile){
    ...
    for(var i=0;i<page.length;i++){
      ...
      cardHtml+='<div class="pba-card-title"><a class="pba-title-link" href="'+esc(v.youtube_url||'#')+'" target="_blank" rel="noopener" style="color:#e6edf3;text-decoration:none">'+esc(v.video_title||'--')+'</a> <span data-vid="'+esc(v.video_id||'')+'" onclick="toggleFav(\''+esc(v.video_id||'')+'\')" style="cursor:pointer;font-size:0.9em">'+(isFav(v.video_id)?'\u2605':'\u2606')+'</span></div>';
    ...
    for(var i=0;i<page.length;i++){
      ...
      html+='<td class="col-title" title="'+esc(v.video_title||'')+'"><a class="pba-title-link" href="'+esc(v.youtube_url||'#')+'" target="_blank" rel="noopener" style="color:#e6edf3;text-decoration:none">'+esc(v.video_title||'--')+'</a></td>';
